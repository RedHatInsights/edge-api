...
func PostMigrateDeleteCustomRepo() (int64, error) {
	...
	result := db.DB.Debug().Exec("DELETE FROM third_party_repos WHERE deleted_at IS NULL AND uuid IS NOT NULL and uuid !='' AND id NOT IN (SELECT third_party_repo_id FROM images_repos);")
	if result.Error != nil {
