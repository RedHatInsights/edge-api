...
func cleanupOrphanInstalledPackagesPlain(gormDB *gorm.DB) error {
	...
	for keepGoing {
		...
		log.WithField("batch_size", batchSize).
			Infof("Deleted %d (= %d total) orphan installed packages in %02f min, records per hour: %02f", tx.RowsAffected, total,
				time.Since(startTime).Minutes(), float64(total)/time.Since(totalStartTime).Hours())
	}
