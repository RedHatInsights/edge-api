...
var (
	...
	host                = config.Get().FDOHostURL
	version             = config.Get().FDOApiVersion
	authorizationSecret = fmt.Sprint("secret ", config.Get().FDOAuthorizationSecret)
)
...
func (c *Client) BatchUpload(ovs []byte, numOfOVs uint) (interface{}, error) {
	...
	req.Header.Add("Authorization", authorizationSecret)
