...
type ImageBuilderClientInterface interface {
	...
	ComposeInstaller(image *models.Image, headers map[string]string) (*models.Image, error)
...
func (c *ImageBuilderClient) ComposeInstaller(image *models.Image, headers map[string]string) (*models.Image, error) {
	pkgs := make([]string, 0)
	req := &ComposeRequest{
		Customizations: &Customizations{
			Packages: &pkgs,
		...
		ImageRequests: []ImageRequest{
			{
				...
				Ostree: &OSTree{
					...
					URL: fmt.Sprintf("%s/%s/%d/repo", c.RepoURL, image.Commit.Account, image.Commit.ID),
				},
				UploadRequest: &UploadRequest{
					Options: make(map[string]string),
					Type:    "aws.s3",
