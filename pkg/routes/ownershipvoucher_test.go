...
var _ = Describe("Ownershipvoucher", func() {
	...
	Context("create ownership vouchers", func() {
		...
		http.HandleFunc(fmt.Sprintf("/management/%s/ownership_voucher", config.Get().FDO.APIVersion), handler)
	...
	Context("delete ownership vouchers", func() {
		...
		http.HandleFunc(fmt.Sprintf("/management/%s/ownership_voucher/delete", config.Get().FDO.APIVersion), handler)
		...
		It("delete request with empty body", func() {
			req, _ := http.NewRequest(http.MethodPost, "/ownership_voucher/delete", bytes.NewBuffer([]byte{}))
			resRec := httptest.NewRecorder()
			req.Header.Add("Content-Type", "application/json")
			req.Header.Add("Accept", "application/json")
			router.ServeHTTP(resRec, req)
		...
		It("delete request with invalid Accept header", func() {
			req, _ := http.NewRequest(http.MethodPost, "/ownership_voucher/delete", bytes.NewBuffer([]byte("invalid")))
			resRec := httptest.NewRecorder()
			req.Header.Add("Content-Type", "application/json")
			req.Header.Add("Accept", "application/xml")
			router.ServeHTTP(resRec, req)
