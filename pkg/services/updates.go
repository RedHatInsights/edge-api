...
func (s *UpdateService) ValidateUpdateDeviceGroup(account string, deviceGroupID uint) (bool, error) {
	...
	result := db.DB.
		...
		Joins(`JOIN Device_Groups_Devices as Device_Groups_Devices ON Device_Groups.id = Device_Groups_Devices.device_group_id`).
		Joins(`JOIN Devices as Devices ON Device_Groups_Devices.device_id = Devices.id`).
		Where("Devices.image_id IS NOT NULL AND Devices.image_id != 0").
		Joins(`JOIN Images as Images ON Devices.image_id = Images.id`).
